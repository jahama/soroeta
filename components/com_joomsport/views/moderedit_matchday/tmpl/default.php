...
<?php
		...
		function submitbutton(pressbutton) {
			...
			var mtime = eval("document.adminForm['match_time[]']");
				if(mtime){
					if(mtime.length){
						for(i=0;i<mtime.length;i++){
							if(!mtime[i].value.test(regE) || mtime[i].value.substr(0,2)>23){
								alert("<?php echo JText::_('BLFA_JSMDNOT7')?>");return;
							}
						}
					}else{
						if(!mtime.value.test(regE) || mtime.value.substr(0,2)>23){
								alert("<?php echo JText::_('BLFA_JSMDNOT7')?>");return;
							}
					}	
				}
		...
		function bl_add_match(){
			...
			//if ((trim(score1) == '' || trim(score2) == '') && tm_played){
				//alert("Please enter text to the field.");return;
			//}

			if ( team1.value == team2.value || (team1.value!='<?php echo $this->tid?>' && team2.value!='<?php echo $this->tid?>')){
			...
			var regE = /[0-2][0-9]:[0-5][0-9]/;
			if(!getObj('match_time_new').value.test(regE) || getObj('match_time_new').value.substr(0,2)>23){
				alert("<?php echo JText::_('BLFA_JSMDNOT7')?>");return;
			}
		...
		<form action="index.php?option=com_joomsport&controller=moder&task=matchday_medit&tid=<?php echo $this->tid;?>&sid=<?php echo $this->s_id;?>&Itemid=<?php echo $Itemid;?>" method="post" name="filtrForm">
			<div align="right"><?php echo JText::_('BLFA_FILTERS')?> :<?php echo $lists['seas_filtr']."".$lists['md_filtr'];?></div>	
		</form>
		...
		<table class="adminlist" id="new_matches" border="0">
			...
			if(count($match)){
				...
				foreach($match as $curmatch){
					...
					$match_link = 'index.php?option=com_joomsport&amp;task=match_medit&amp;cid[]='.$curmatch->id.'&amp;controller=moder&amp;sid='.$this->s_id.'&amp;tid='.$this->tid.'&amp;Itemid='.$Itemid;
		...
		<table class="adminlist">
			...
			<tr>
				...
				<th>
					...
					<input type="button" value="Add" onClick="bl_add_match();" />	
				</th>
