...
?>
<div align="right">
<div style="float:right;padding:0px 5px;width:32px;"><a href="<?php echo "index.php?option=com_joomsport&controller=moder&task=matchday_medit&mid=".$row->m_id."&sid=".$s_id."&tid=".$this->tid."&Itemid=".$Itemid;?>" class="toolbar" ><span class="icon-32-cancel"></span><?php echo JText::_('BLFA_CLOSE')?></a></div>
...
<?php
		...
		function submitbutton(pressbutton) {
			...
			if(pressbutton == 'match_apply' || pressbutton == 'match_save'){
			
				var regE = /[0-2][0-9]:[0-5][0-9]/;
				if(!getObj('m_time').value.test(regE) || getObj('m_time').value.substr(0,2)>23){
		...
		<table class="jsnoborders">
			...
			<tr>
				...
				<td>
					<input type="text" maxlength="5" size="10" id="m_time" name="m_time" value="<?php echo substr($row->m_time,0,5);?>" />
			...
			<tr>
				...
				<td>
					<?php echo $editor->display( 'match_descr',  htmlspecialchars($row->match_descr, ENT_QUOTES), '100%', '300', '60', '20', array('pagebreak', 'readmore') ) ;  ?>
				</td>
			</tr>
