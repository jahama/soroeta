 ...
 * @since 1.0
 */
class bleagueViewmoderedit_match extends JView
{
	function display($tpl = null)
	{
		...
		$sid	= $mainframe->getUserStateFromRequest( $option.'.sid', 'sid', 0, 'int' );
	...
	/*$query = "SELECT * FROM #__bl_matchday  WHERE s_id = ".($row->s_id?$row->s_id:$season_id)." ORDER BY m_name";
	$db->setQuery($query);
	...
	$query = "SELECT id,CONCAT(first_name,' ',last_name) as p_name FROM #__bl_players WHERE team_id = ".$tid." ORDER BY first_name,last_name";
	$db->setQuery($query) ;
	$players_rat = $db->loadObjectList();
	$is_playerq[] = JHTML::_('select.option',  0, JText::_('Select Player'), 'id', 'p_name' ); 
	$ev_pl = array_merge($is_playerq,$players_rat);
	$lists['players_rating'] = JHTML::_('select.genericlist',   $ev_pl, 'plrating', 'class="inputbox" size="1"', 'id', 'p_name', 0);
